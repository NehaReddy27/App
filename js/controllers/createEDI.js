...
.controller('SlideBar', ['$location','$rootScope','$scope','$window','customHttp',function ($location,$rootScope,$scope,$window,customHttp){	
	
	$scope.slide = false;
	$rootScope.slide1 = false;
	
	$scope.sidebar = function(){
	if($scope.slide == false){
		$scope.slide = true;
		$rootScope.slide1 = true;
	}
	else if($scope.slide == true){
		$scope.slide = false;
		$rootScope.slide1 = false;
	}	
	}
...
.controller('AgencyAndVersionCtrl', ['$location','$rootScope','$scope','$window','customHttp',function ($location,$rootScope,$scope,$window,customHttp){	
	$scope.state=0;
	$scope.loading=false;
...
.controller('VersionCtrl', ['$location','$rootScope','$scope','$window','customHttp',function ($location,$rootScope,$scope,$window,customHttp){	
	...
	$scope.search2 = function(index,x)
	{
		var res1=[];
		var res2=[];
		res2=$scope.Version;
...
.controller('TransactionSetCtrl', ['$location','$rootScope','$scope','$window','customHttp',function ($location,$rootScope,$scope,$window,customHttp){	
...
.controller('SegmentUsageCtrl', ['$location','$rootScope','$scope','customHttp',function ($location,$rootScope,$scope,customHttp){	
	...
	$scope.slide2 = true;
	$scope.sidebar = function(){
	if($scope.slide2 == false){
		$scope.slide2 = true;
		
	}
	else if($scope.slide2 == true){
		$scope.slide2 = false;
...
.controller('ElementUsageDefsCtrl', ['$location','$rootScope','$scope','customHttp',function ($location,$rootScope,$scope,customHttp){	
		...
		customHttp.request(params,'/api/elementUsageDefs/getWithCode','POST',function(data){
			...
			if(data.status==true)
			{
				...
				for(i=0;i<resultRecord.length;i++)
				{
					...
					$scope.setcodehoverstatus[resultRecord[i]['Position']]=false;
				}
	...
	{
		$scope.setcodehoverstatus[pos]=true;
	}
	
	$scope.leave = function(pos)
	{
		$scope.setcodehoverstatus[pos]=false;
	}
